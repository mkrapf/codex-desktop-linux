...
impl HyprlandConfigProvider {
    fn focus_detail(self) -> &'static str {
        match self {
            Self::Lua => "Lua config provider detected; focus tries hl.dsp.focus first",
            Self::Legacy => "legacy config provider detected; focus retains focuswindow fallback",
            Self::Unknown => "config provider was not reported; both focus dispatchers are tried",
        }
    }
}
